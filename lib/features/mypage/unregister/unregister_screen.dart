  ...
  Future<void> _handleUnregister(BuildContext context) async {
    ...
    if (result.status == UnregisterStatus.success) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('정상적으로 탈퇴 처리되었습니다.')));
      if (context.mounted) {
    ...
    if (result.status == UnregisterStatus.unauthorized) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('인증이 필요합니다. 다시 로그인해주세요.')));
      if (context.mounted) {
        Navigator.of(context)
            .pushNamedAndRemoveUntil('/login', (r) => false);
      }
    ...
    final msg =
    result.message.isNotEmpty ? result.message : '탈퇴 실패: ${result.code}';
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
  }

  @override
  Widget build(BuildContext context) {
    // ✅ 수정된 부분: 현재 테마를 가져오기 위해 변수 선언
          ...
          child: Column(
            ...
            children: [
              ...
              TextButton(
                ...
                // ✅ 수정된 부분: TextStyle에 테마의 텍스트 색상 적용
                child: Text('탈퇴 시 처리 안내 보기',
                    style: TextStyle(
                      fontSize: 16,
